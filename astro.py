# astro.py (в корне вашего проекта)
import os
import logging
import hashlib
import random
...
# На Vercel эти переменные будут установлены автоматически.
load_dotenv()
...
if not MONGO_URI:
    raise ValueError("Environment variable MONGO_URI is not set. Please provide your MongoDB Atlas connection string.")
...
TEXTS = {
    "ru": {
        ...
        "planet_uranus": "Уран", "planet_neptune": "Нептун", "planet_crystal": "Кристалл (по умолчанию)",
...
async def get_user_data(user_id: int):
    if users_collection:
        ...
        return user_data if user_data else {"_id": user_id, "sign": "aries", "lang": "ru", "birth_date": None}
    return {"_id": user_id, "sign": "aries", "lang": "ru", "birth_date": None}
...
async def update_user_data(user_id: int, key: str, value):
    if users_collection:
        ...
        logger.warning(f"MongoDB коллекция не инициализирована, данные пользователя {user_id} не будут сохранены.")
    ...
    async def _generate_aspect_description(aspect: str, score: int, rng: random.Random, lang: str) -> str:
        descriptions = {
            ...
            TEXTS[lang]["horoscope_finance"]: {
                ...
                (1, 2): {
                    "ru": ["🚫 Лучше воздержаться от любых серьезных финансовых операций. Высока вероятность потерь или обмана. Будьте максимально осторожны и не доверяйте сомнительным предложениям.",
            ...
            TEXTS[lang]["horoscope_health"]: {
                ...
                (1, 2): {
                    "ru": ["🚨 Вашему здоровью требуется повышенное внимание. Избегайте стрессов и позаботьтесь о своем организме с особой нежностью. Если что-то беспокоит, лучше обратиться к специалисту.",
...
async def show_ads(user_id: int):
    try:
        if ADSGRAM_API_KEY and random.randint(1, 5) == 1:
            builder = InlineKeyboardBuilder()
            builder.button(text=await get_text_async(user_id, "ad_button"), url="https://example.com")
...
# Функции on_startup и on_shutdown перенесены в api/index.py,
# а scheduled_tasks в api/cron.py
# (Удалены из этого файла, чтобы избежать циклической зависимости или повторного запуска)
